    ...
    parseStatement(ast) {
        ...
        recast.visit(ast, {
                    ...
                    if (d.type === 'VariableDeclarator' && d.init != null) {
                        ...
                        if (d.init.type === 'CallExpression' &&
                            ...
                            if (d.init.arguments.length !== 2)
                                throw new Error(`Array requires exactly 2 arguments`);
                            const arg0 = d.init.arguments[0];
                            const arg1 = d.init.arguments[1];
                            if (arg0.type !== 'Literal' && arg0.type !== 'ObjectExpression') {
                                throw new Error(`Array must be typed using literal or object expression: at (${((_a = arg0.loc) === null || _a === void 0 ? void 0 : _a.start.line, (_b = arg0.loc) === null || _b === void 0 ? void 0 : _b.start.column)})`);
                            }
                            if (arg1.type !== 'Literal' || typeof arg1.value !== 'number') {
                                throw new Error(`Array must have number as second argument: at (${((_c = arg0.loc) === null || _c === void 0 ? void 0 : _c.start.line, (_d = arg0.loc) === null || _d === void 0 ? void 0 : _d.start.column)})`);
                            }
                            const argType = parseExpression(arg0).type;
                            if (argType.name !== 'double' && argType.name !== 'object')
                                throw new Error();
                        ...
                        if (expr.type.name === 'object' && !expr.type.orphan) {
                            if (expr.type.name === 'object' && !expr.type.orphan) {
                                throw new Error(`Cannot reasign objects (did you mean to use this.copy(obj)): for '${expr.val}' at (${(_f = (_e = d.loc) === null || _e === void 0 ? void 0 : _e.start.line) !== null && _f !== void 0 ? _f : '?'},${(_h = (_g = d.loc) === null || _g === void 0 ? void 0 : _g.start.column) !== null && _h !== void 0 ? _h : '?'})' for '${expr.val}'`);
                            }
                        }
            ...
            visitIfStatement(path) {
                val = `if (${parseExpression(path.node.test).val}) ${parseStatement(path.node.consequent)} ${path.node.alternate ? `else ${parseStatement(path.node.alternate)}` : ''}`;
                return false;
            },
            visitForStatement(path) {
                val = `for(${path.node.init ? parseStatement(path.node.init) : ';'} ${path.node.test ? parseExpression(path.node.test).val : ''}; ${path.node.update ? parseExpression(path.node.update).val : ''}) ${parseStatement(path.node.body)}`;
                return false;
            },
            visitExpressionStatement(path) {
                if (path.node.expression.type === 'AssignmentExpression') {
                    val = `${parseExpression(path.node.expression.left).val} ${path.node.expression.operator} ${parseExpression(path.node.expression.right).val};`;
                }
                else {
