    ...
    parseExpression(ast, context, ignoreType = false) {
        ...
        recast.visit(ast, {
            ...
            visitMemberExpression(path) {
                ...
                if (path.node.computed) {
                    ...
                    if (object.type.name !== 'array')
                        throw new Error(`Expected array got: ${object.type.name}`);
                    ...
                    if (path.node.property.type !== 'Identifier')
                        throw new Error('Expected Identifier in member expression');
                    ...
                    if (left.type != null && left.type.name === 'object') {
                        ...
                        throw new Error(`Bad member expression at (${(_a = path.node.property.loc) === null || _a === void 0 ? void 0 : _a.start.line}, ${(_b = path.node.property.loc) === null || _b === void 0 ? void 0 : _b.start.column})`);
                    }
            ...
            visitCallExpression(path) {
                ...
                if (pcallee.type === 'MemberExpression' &&
                    pcallee.object.type === 'ThisExpression' &&
                    pcallee.property.type === 'Identifier' &&
                    pcallee.property.name === 'copy') {
                    if (path.node.arguments.length !== 1)
                        throw new Error(`Copy requires exactly 1 argument`);
                    const arg = parseExpression(path.node.arguments[0]);
                    if (arg.type.name !== 'object')
                        throw new Error(`Copy requires object argument`);
                ...
                if (pcallee.type === 'MemberExpression' &&
                    ...
                    throw new Error(`Unhandled array creation at (${(_a = pcallee.loc) === null || _a === void 0 ? void 0 : _a.start.line}, ${(_b = pcallee.loc) === null || _b === void 0 ? void 0 : _b.start.column})`);
                }
            ...
            visitObjectExpression(path) {
                const props = path.node.properties.map((prop) => {
                    if (prop.type !== 'Property')
                        throw new Error('Property must be an "Property"');
                    if (prop.key.type !== 'Identifier')
                        throw new Error('Property must have Identifier as a key"');
                ...
                const properties = props.reduce((obj, { key, value }) => (Object.assign(Object.assign({}, obj), { [key]: value.type })), {});
